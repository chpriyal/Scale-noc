...
def run_net( ifmap_sram_size=1,
    ...
    for row in param_file:
        ...
        ifmap_h = math.ceil((int(elems[1]))//math.sqrt(num_systolic_arrays)) + filt_h - 1
        ifmap_w = math.ceil((int(elems[2]))//math.sqrt(num_systolic_arrays)) + filt_w - 1

        num_channels = int(elems[5])
        num_filters =  int(elems[6])
        # print(num_filters)
        # if(int(elems[6])%num_systolic_arrays != 0):
