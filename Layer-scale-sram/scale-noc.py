...
# flags.DEFINE_string("num_sys_arrays",")

class scale:
    def __init__(self, sweep = False, save = False, num_pe = 128*128):
        self.sweep = sweep
    ...
    def run_once(self, dataflow, ar_h_min, ar_w_min,  num_sys_arrays, net_name):
        ...
        print("******************* SCALE SIM **********************")
        print("====================================================")
        print("Array Size: \t" + str(ar_h_min) + "x" + str(ar_w_min))
        ...
        r.run_net(  ifmap_sram_size  = int(int(self.isram_min)/num_sys_arrays),
                    filter_sram_size = int(int(self.fsram_min)/num_sys_arrays),
                    ofmap_sram_size  = int(int(self.osram_min)/num_sys_arrays),
                    array_h = int(ar_h_min),
                    array_w = int(ar_w_min),
                    net_name = net_name,
                    data_flow = dataflow,
                    topology_file = self.topology_file,
                    offset_list = offset_list,
                    num_systolic_arrays = num_sys_arrays
                )
        self.cleanup(net_name,net_name,dataflow)
        print("************ SCALE SIM Run Complete ****************")


    def cleanup(self,net_name, run_name,dataflow):
        ...
        if run_name == "":
            path = "./outputs/" + net_name +"_"+ dataflow
        else:
            path = "./outputs/" + run_name
    ...
    def run_sweep(self):
        ...
        arr_h_list = all_arr_dim_list[3:9]
        arr_w_list = all_arr_dim_list[3:9]
        ...
        for df in data_flow_list:
            ...
            for i in range(len(arr_h_list)):
                ...
                # if(self.arr_h*)
                self.num_sys_arrays = (self.num_pe/(self.ar_h_min * self.ar_h_min))
                self.run_name = net_name + "_" + df + "_" + str(self.ar_h_min) + "x" + str(self.ar_w_min)

                arglist.append((df,self.ar_h_min, self.ar_w_min,self.num_sys_arrays, self.run_name))
        print("Starting processing")
        process_pool = multiprocessing.Pool(18)
        process_pool.starmap(self.run_once, arglist)
        

def main(argv):
    

    s = scale(save = False, sweep = True, num_pe = 256*256)
